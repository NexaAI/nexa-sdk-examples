
import SwiftData
import Foundation
import NexaAI

@Observable
@MainActor
class ModelManager {

    private(set) var model: Model?
    let configManager: ModelConfigManager
    let modelPersistence: ModelPersistence
    var currentModelInfo: ModelInfo? {
        didSet {
            configManager.saveLastModelInfo(currentModelInfo)
            modelPersistence.updateLastUseTime(of: currentModelInfo)
            reload()
        }
    }

    private var isGenerating: Bool = false
    var isLoadingModel: Bool = false

    private(set) var forceRefesh: Bool = false

    init(
        model: Model? = nil,
        modelConfigManager: ModelConfigManager,
        modelPersistence: ModelPersistence
    ) {
        self.model = model
        self.configManager = modelConfigManager
        self.currentModelInfo = modelConfigManager.loadLastModelInfo()
        self.modelPersistence = modelPersistence
    }

    var isLoaded: Bool {
        model?.isLoaded ?? false
    }

    func reload() {
        forceRefesh.toggle()
    }

    func unload() async {
        self.model = nil
    }

    func load(modelType: NexaAI.ModelType, options: ModelOptions) async throws {
        isLoadingModel = true
        defer { isLoadingModel = false }
        await unload()
        self.model = modelType == .llm ? LLMLlama() : VLMLlama()
        try await self.model?.load(options)
    }

    func stopGeneration() {
        model?.stopStream()
    }

    func generateAsyncStream(from messages: [Message], options: GenerationOptions, systemPrompt: String? = nil) async throws -> AsyncThrowingStream<String, any Error> {
        guard let model else {
            throw ModelError.notLoad
        }

        var msgs = [ChatMessage]()
        msgs.append(.init(role: .system, content: systemPrompt ?? configManager.systemPrompt))
        msgs.append(contentsOf: messages.map {
            let images = $0.images.map { $0.path() }
            let audios = $0.audios.map { $0.path() }
            return ChatMessage(role: $0.isUser ? .user : .assistant, content: $0.content, images: images, audios: audios)
        })
        return try await model.generateAsyncStream(messages: msgs, options: options)
    }
    
    func reset() async {
        stopGeneration()
        await model?.reset()
    }

    func getProfileData() async -> ProfileData? {
        await model?.lastProfileData
    }

    enum ModelError: LocalizedError {
        case notLoad
    }
}
