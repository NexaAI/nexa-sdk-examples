...
class ChatViewModel {
    ...
    private func _generationStream() async throws {
        ...
        let options = GenerationOptions(config: .init(maxTokens: 4096), templateOptions: .init(enableThinking: false))
        let stream = try await modelManager.generateAsyncStream(messages: genMsgs, options: options)
