...
class ModelManager {
    ...
    func generateStream(messages: [Message], options: GenerationOptions = .init()) async throws -> String {
    ...
    func generateAsyncStream(messages: [Message], options: GenerationOptions = .init()) async throws -> AsyncThrowingStream<String, any Error> {
        guard let model else {
            throw ModelError.notLoad
        }
        var msgs = [ChatMessage]()
